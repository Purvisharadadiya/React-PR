...
const Womenwear = () => {
  ...
  useEffect(() => {
    if (products && products.length > 0) {
      const womenData = products.filter((item) => item.mainCategory === "Women");
      setSortedProducts(womenData);
    }
  }, [products]);
  ...
  const handleGenreFilter = (e) => {
    ...
    if (filter === "low") sorted.sort((a, b) => a.price - b.price);
    else if (filter === "high") sorted.sort((a, b) => b.price - a.price);
    setSortedProducts(sorted);
  };

 
  const applyFilters = (categories, brands, patterns, sizes) => {
    let filtered = products?.filter((p) => p.mainCategory === "Women") || [];
  ...
  const handleClear = () => {
    setSortedProducts(products?.filter((p) => p.mainCategory === "Women") || []);
    setGenreFilter("");
  ...
  const categoryList = ["Saree", "Dress", "Kurti", "Top", "Jeans"];
  const brandList = ["Zara", "H&M", "Forever21", "Levis"];
  const patternList = ["Floral", "Printed", "Solid", "Striped"];
                ...
                {categoryList.map((cat) => (
                  <li key={cat}>
                    ...
                    <small className="count">
                      {products?.filter(
                        (p) => p.mainCategory === "Women" && p.category === cat
                      ).length || 0}
                    </small>
                  </li>
                ))}
              ...
              <ul className="filter-list mt-3">
                {brandList.map((brand) => (
                  <li key={brand}>
                    ...
                    <small className="count">
                      {products?.filter(
                        (p) => p.mainCategory === "Women" && p.brand === brand
                      ).length || 0}
                    </small>
                  </li>
                ))}
                ...
                {patternList.map((pattern) => (
                  <li key={pattern}>
                    ...
                    <small className="count">
                      {products?.filter(
                        (p) =>
                          p.mainCategory === "Women" && p.pattern?.includes(pattern)
                      ).length || 0}
                    </small>
                  </li>
                ))}
              ...
              <ul className="filter-list mt-3">
                {sizeList.map((size) => (
                  <li key={size}>
                    ...
                    <small className="count">
                      {products?.filter(
                        (p) => p.mainCategory === "Women" && p.size?.includes(size)
                      ).length || 0}
                    </small>
                  </li>
                ))}
